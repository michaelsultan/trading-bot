...
function quantPick(
  ...
  const MIN_SCORE = 35;                // Raised from 20 — filter out junk trades  // Config C: very low bar — take more trades for higher volume
...
async function runFullCycle(scanTriggers: ScanTrigger[] = []): Promise<Response> {
  ...
  for (const decision of decisions) {
    ...
    if (decision.action === "BUY" && etHour >= 12 && etHour < 14) {
      ...
      const LUNCH_MIN_SCORE = 45;          // Raised from 25 — higher bar during lunch lull  // Config C: 25 (was 60 — too aggressive, killed all midday trades)
